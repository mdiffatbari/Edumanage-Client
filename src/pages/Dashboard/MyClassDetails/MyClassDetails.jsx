...
const MyClassDetails = () => {
  ...
  const handleAddAssignment = async () => {
    ...
    try {
      ...
      await axios.patch(`http://localhost:3000/classes/${id}`, {
        assignment: (classInfo.assignment || 0) + 1
      });

      setClassInfo(prev => ({ ...prev, assignment: (prev.assignment || 0) + 1 }));
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        <div className="card bg-base-100 shadow-md p-4 border">
          ...
          <p className="text-3xl font-bold">{classInfo.enrolled || 0}</p>
        </div>

        <div className="card bg-base-100 shadow-md p-4 border">
          <h2 className="text-xl font-semibold mb-2">Total Assignments</h2>
          <p className="text-3xl font-bold">{classInfo.assignment || 0}</p>
        </div>
